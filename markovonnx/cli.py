...
def cmd_train(args: argparse.Namespace) -> None:
    ...
    if export_c:
        ...
        if getattr(args, "platformio", False):
            _write_platformio_files(export_c, "markov-c")
...
def cmd_export(args: argparse.Namespace) -> None:
    ...
    platformio = getattr(args, "platformio", False)
    ...
    if fmt == "markov-c":
        ...
        if platformio:
            _write_platformio_files(out, fmt)
    elif fmt == "hmm-c":
...
def cmd_train_hmm(args: argparse.Namespace) -> None:
    ...
    if export_c:
        ...
        print(f"C header written: {export_c}")
        if getattr(args, "platformio", False):
            _write_platformio_files(export_c, "hmm-c")


def _write_platformio_files(out_h: str, fmt: str) -> None:
    """Generate platformio.ini and src/main.cpp next to the exported header.

    Args:
        out_h: Path to the ``.h`` header file that was exported.
        fmt: Either ``"markov-c"`` or ``"hmm-c"``.
    """
    import os

    base_dir = os.path.dirname(os.path.abspath(out_h))
    ini_path = os.path.join(base_dir, "platformio.ini")
    src_dir = os.path.join(base_dir, "src")
    os.makedirs(src_dir, exist_ok=True)
    cpp_path = os.path.join(src_dir, "main.cpp")

    ini_content = """; Generated by markovonnx
[env:esp32dev]
...
"""
    ...
    h_name = os.path.basename(out_h)

    if fmt == "markov-c":
        cpp_content = f"""// Generated by markovonnx — markov_sample example sketch
#include <Arduino.h>
#include "../{h_name}"

void setup() {{
  Serial.begin(115200);
  // Example: sample from context key 0
  uint64_t key = 0;
  float r = (float)random(0, 256) / 256.0f;
  int token_id = markov_sample(markov_lookup(key), r);
...
"""
        ...
        cpp_content = f"""// Generated by markovonnx — hmm_forward_step example sketch
#include <Arduino.h>
#include "../{h_name}"
...
void setup() {{
  ...
  // Initialize with first observation id 0
  hmm_forward_init(0, alpha);
  // Step with next observation id 1
  hmm_forward_step(1, alpha);
  int best = hmm_best_state(alpha);
...
"""
    ...
    print(f"PlatformIO project written: {ini_path}")
    print(f"  Sketch: {cpp_path}")
...
def main() -> None:
    ...
    p_train.add_argument("--platformio", action="store_true", help="Generate platformio.ini + src/main.cpp alongside --export-c")
    ...
    p_export.add_argument("--platformio", action="store_true", help="Generate platformio.ini + src/main.cpp alongside the header")
