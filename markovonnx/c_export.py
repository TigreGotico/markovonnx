...
def export_markov_c_header(
    ...
    * ``markov_sample()`` — inline CDF walk returning a token id.
    ...
    If the chain was trained with ``backoff=True`` and has a lower-order model,
    a second set of arrays (``MARKOV_KEYS_LOWER``, ``MARKOV_PROBS_LOWER``) and
    a ``markov_lookup_backoff()`` helper are also emitted.  Only one level of
    backoff is exported; deeper chains are silently ignored.
    ...
    lower_rows: Optional[List[Tuple[int, np.ndarray]]] = None
    if chain._lower is not None:
        lower_rows = _collect_rows(chain._lower, V, quantize)

    header = _render_header(rows, vocab, V, order, quantize, progmem, lower_rows)
...
def _render_header(
    ...
    quantize: bool,
    progmem: bool,
    lower_pairs: Optional[List[Tuple[int, np.ndarray]]] = None,
) -> str:
    ...
    Args:
        ...
        lower_pairs: Optional sorted pairs for the lower-order backoff chain.
    ...
    has_backoff = lower_pairs is not None
    has_backoff_str = str(has_backoff).lower()
    lower_order = order - 1 if has_backoff else 0
    # Mask to extract the lower-order key from the full key:
    # drop the highest 16-bit slot by masking to (order-1)*16 bits
    lower_mask = (1 << (16 * lower_order)) - 1 if has_backoff else 0
...
"""
    ...
    if has_backoff:
        n_lower = len(lower_pairs)
        lower_keys_str = _fmt_keys(lower_pairs)
        lower_probs_str = _fmt_probs_uint8(lower_pairs) if quantize else _fmt_probs_float(lower_pairs)
        header += f"""
// Backoff chain (order={lower_order}) — used when full-order context is unseen.
#define MARKOV_SPARSE_ROWS_LOWER {n_lower}

static const uint64_t{prog_attr} MARKOV_KEYS_LOWER[MARKOV_SPARSE_ROWS_LOWER] = {{
  {lower_keys_str}
}};

static const {prob_type}{prog_attr} MARKOV_PROBS_LOWER[MARKOV_SPARSE_ROWS_LOWER][MARKOV_VOCAB_SIZE] = {{
{lower_probs_str}
}};

// Backoff lookup: searches lower-order chain using the last {lower_order} token(s) of key.
static inline const {prob_type}* markov_lookup_lower(uint64_t key) {{
  uint64_t lower_key = key & 0x{lower_mask:016X}ULL;
  int lo = 0, hi = MARKOV_SPARSE_ROWS_LOWER - 1;
  while (lo <= hi) {{
    int mid = lo + (hi - lo) / 2;
    if (MARKOV_KEYS_LOWER[mid] == lower_key) return MARKOV_PROBS_LOWER[mid];
    if (MARKOV_KEYS_LOWER[mid] < lower_key) lo = mid + 1;
    else hi = mid - 1;
  }}
  return 0;
}}

// Backoff sample: tries full-order lookup, falls back to lower-order, then returns 0.
static inline int markov_sample_backoff(uint64_t key, float r) {{
  const {prob_type}* row = markov_lookup(key);
  if (!row) row = markov_lookup_lower(key);
...
def export_hmm_c_header(
    ...
    * ``hmm_viterbi()`` — inline Viterbi decoder.  Caller allocates ``delta``
    ...
    log_pi = np.log(np.asarray(hmm.pi, dtype=np.float32) + 1e-30)
    log_A = np.log(np.asarray(hmm.A, dtype=np.float32) + 1e-30)
    log_B = np.log(np.asarray(hmm.B, dtype=np.float32) + 1e-30)
    ...
    header = _render_hmm_header(log_pi, log_A, log_B, obs_vocab, state_vocab, S, O, progmem)
